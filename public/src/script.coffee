...
window.helpers =
	titleCase: (str) ->
		str.replace(/([A-Z])/g, " $1").replace /(^[a-z])/, (s, group) ->
			group.toUpperCase()

	html: (el) ->
		if el.outerHTML then el.outerHTML
		else $('<div/>').append(el).html()

	pad: (str, count, char = " ") ->
		return str if str.length >= count
		padding = count - str.length
		paddingString = ""
		paddingString = "#{char}#{paddingString}" for x in [0...padding]
		"#{str}#{paddingString}"

	shorten: (str, count) ->
		return str if str.length <= count + 3
		helpers.html $("<span title='#{str}'>#{str.substr 0, count}...</span>")

	getField: (k, v) ->
		data = k: k, v: v
		if hexColorRegex.test v
			data.rgba = new ColorModel(color: v).rgba()
			tplId = "#colorInput"
		else if $.isNumeric v
			tplId = "#numericInput"
		else
...
class RulesView extends Backbone.View
	...
	render: =>
		...
		@model.get("settings").each (rule, index) =>
			if index is 0 then @renderMainSettings rule
			else @renderRule rule
		@
...
window.ColorModel = class ColorModel extends Backbone.Model
	...
	rgba: =>
		if @get("color") is "#ffffff00" then return ""
